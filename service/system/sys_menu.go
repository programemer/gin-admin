...
func (menuService *MenuService) getMenuTreeMap(authorityId uint) (treeMap map[uint][]entity.SysMenu, err error) {
...
func (menuService *MenuService) getChildrenList(menu *entity.SysMenu, treeMap map[uint][]entity.SysMenu) (err error) {
...
func (menuService *MenuService) getBaseChildrenList(menu *entity.SysBaseMenu, treeMap map[uint][]entity.SysBaseMenu) (err error) {
...
func (menuService *MenuService) AddBaseMenu(menu entity.SysBaseMenu) error {
	if !errors.Is(global.GVA_DB.Where("name = ?", menu.Name).First(&entity.SysBaseMenu{}).Error, gorm.ErrRecordNotFound) {
...
func (menuService *MenuService) getBaseMenuTreeMap(authorityID uint) (treeMap map[uint][]entity.SysBaseMenu, err error) {
	...
	if global.GVA_CONFIG.System.UseStrictAuth && parentAuthorityID != 0 {
		var authorityMenus []entity.SysAuthorityMenu
...
func (menuService *MenuService) GetBaseMenuTree(authorityID uint) (menus []entity.SysBaseMenu, err error) {
	treeMap, err := menuService.getBaseMenuTreeMap(authorityID)
	menus = treeMap[0]
	for i := 0; i < len(menus); i++ {
...
func (menuService *MenuService) AddMenuAuthority(menus []entity.SysBaseMenu, adminAuthorityID, authorityId uint) (err error) {
...
func (menuService *MenuService) UserAuthorityDefaultRouter(user *entity.SysUser) {
	var menuIds []string
	err := global.GVA_DB.Model(&entity.SysAuthorityMenu{}).Where("sys_authority_authority_id = ?", user.AuthorityId).Pluck("sys_base_menu_id", &menuIds).Error
	if err != nil {
		return
	}
